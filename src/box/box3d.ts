// Box3d — 3D axis-aligned bounding box, float64 components.
//
// Backed by a `Float64Array(6)` flat-storing
...
export class Box3d {
  ...
  constructor(
    ...
    this._data = new Array<number>(6).fill(0);
    this._data[0] = minX;
    this._data[1] = minY;
    this._data[2] = minZ;
    this._data[3] = maxX;
  ...
  static viewOnto(buffer: ArrayBufferLike, byteOffset: number): Box3d {
    const b = Object.create(Box3d.prototype) as { _data: number[] };
  ...
  static get empty(): Box3d {
    return new Box3d(Infinity, Infinity, Infinity, -Infinity, -Infinity, -Infinity);
  }

  static fromMinMax(min: V3d, max: V3d): Box3d {
    return new Box3d(min.x, min.y, min.z, max.x, max.y, max.z);
  }

  static fromCenterRadius(center: V3d, radius: number): Box3d {
    return new Box3d(
  ...
  static fromPoints(points: Iterable<V3d>): Box3d {
  ...
  static fromBoxes(boxes: Iterable<Box3d>): Box3d {
    let mnx = Infinity, mny = Infinity, mnz = Infinity;
    let mxx = -Infinity, mxy = -Infinity, mxz = -Infinity;
  ...
  get min(): V3d { return new V3d(this._data[0]!, this._data[1]!, this._data[2]!); }
  set min(v: V3d) { this._data[0] = v.x; this._data[1] = v.y; this._data[2] = v.z; }
  get max(): V3d { return new V3d(this._data[3]!, this._data[4]!, this._data[5]!); }
  set max(v: V3d) { this._data[3] = v.x; this._data[4] = v.y; this._data[5] = v.z; }
  ...
  contains(p: V3d | Box3d): boolean {
    if (p instanceof Box3d) {
      return (
  ...
  intersects(other: Box3d): boolean {
    return (
  ...
  intersection(other: Box3d): Box3d {
    return new Box3d(
  ...
  union(other: Box3d): Box3d { return this.extend(other); }

  equals(other: unknown): boolean {
    if (this === other) return true;
    if (!(other instanceof Box3d)) return false;
    for (let i = 0; i < 6; i++) {
  ...
  approxEqual(other: Box3d, eps: number): boolean {
    for (let i = 0; i < 6; i++) {
  ...
  toString(): string {
    return `Box3d([${this._data[0]}, ${this._data[1]}, ${this._data[2]}], [${this._data[3]}, ${this._data[4]}, ${this._data[5]}])`;
  }
