// M34d — 3 rows x 4 cols float64 matrix. Row-major storage.
...
export class M34d {
  static readonly __aardworxMathBrand: "M34d" = "M34d";
  ...
  static viewOnto(buffer: ArrayBufferLike, byteOffset: number): M34d {
    const m = Object.create(M34d.prototype) as { _data: Float64Array };
  ...
  static readonly zero: M34d = new M34d();

  static fromRows(rows: V4d[]): M34d {
    const m = new M34d();
  ...
  static fromArray(flat: ArrayLike<number>): M34d {
    const m = new M34d();
    for (let i = 0; i < COMPONENT_COUNT; i++) m._data[i] = flat[i]!;
    return m;
  }

  static copy(other: M34d): M34d {
    const m = new M34d();
  ...
  col(c: number): V3d {
    return new V3d(this._data[0 + c]!, this._data[4 + c]!, this._data[8 + c]!);
  }
  ...
  mul(scalar: number): M34d;
  mul(v: V4d): V3d;
  mul(other: V4d | number): M34d | V3d {
  ...
  transpose(): M43d {
    ...
    // dst M43d has 4 rows, 3 cols; row-major dst index = row * 3 + col_dst.
  ...
  equals(other: unknown): boolean {
    ...
    if (!(other instanceof M34d)) return false;
  ...
  approxEqual(other: M34d, eps: number): boolean {
    for (let i = 0; i < COMPONENT_COUNT; i++) {
  ...
  toString(): string {
    return `M34d(${Array.from(this._data).join(", ")})`;
  }
  ...
  *[Symbol.iterator](): Iterator<number> {
  ...
  static addInto(a: M34d, b: M34d, target: M34d): M34d {
    for (let i = 0; i < COMPONENT_COUNT; i++) target._data[i] = a._data[i]! + b._data[i]!;
    return target;
  }

  static subInto(a: M34d, b: M34d, target: M34d): M34d {
    for (let i = 0; i < COMPONENT_COUNT; i++) target._data[i] = a._data[i]! - b._data[i]!;
    return target;
  }

  static mulInto(a: M34d, b: V4d, target: V3d): V3d;
  static mulInto(a: M34d, b: number, target: M34d): M34d;
  static mulInto(a: M34d, b: V4d | number, target: M34d | V3d): M34d | V3d {
    if (typeof b === "number") {
      const t = target as M34d;
  ...
  static copyInto(from: M34d, target: M34d): M34d {
