// M42d — 4 rows x 2 cols float64 matrix. Row-major storage.
// Notation matches Aardvark.Base: M{rows}{cols}{elem}.
// See m44f.ts for conventions.

import { combineHash, hashNumber } from "../internal/hash.js";
import { V2d } from "../vector/v2d.js";
import { V4d } from "../vector/v4d.js";
import { M24d } from "./m24d.js";

const ROWS = 4;
const COLS = 2;
const COMPONENT_COUNT = ROWS * COLS;
const BYTES = COMPONENT_COUNT * 8;

export class M42d {
  static readonly __aardworxMathBrand: "M42d" = "M42d";

  /** @internal */
  readonly _data: Float64Array;

  constructor() {
    this._data = new Float64Array(COMPONENT_COUNT);
  }

  static viewOnto(buffer: ArrayBufferLike, byteOffset: number): M42d {
    const m = Object.create(M42d.prototype) as { _data: Float64Array };
    m._data = new Float64Array(buffer, byteOffset, COMPONENT_COUNT);
    return m as M42d;
  }

  static readonly componentCount = COMPONENT_COUNT;
  static readonly byteSize = BYTES;
  static readonly rows = ROWS;
  static readonly cols = COLS;

  static readonly zero: M42d = new M42d();

  static fromRows(rows: V2d[]): M42d {
    const m = new M42d();
    for (let r = 0; r < ROWS; r++) {
      const row = rows[r]!;
      for (let c = 0; c < COLS; c++) m._data[r * COLS + c] = row._data[c]!;
    }
    return m;
  }

  static fromCols(cols: V4d[]): M42d {
    const m = new M42d();
    for (let c = 0; c < COLS; c++) {
      const col = cols[c]!;
      for (let r = 0; r < ROWS; r++) m._data[r * COLS + c] = col._data[r]!;
    }
    return m;
  }

  /** Build from a flat row-major array. */
  static fromArray(flat: ArrayLike<number>): M42d {
    const m = new M42d();
    for (let i = 0; i < COMPONENT_COUNT; i++) m._data[i] = flat[i]!;
    return m;
  }

  static copy(other: M42d): M42d {
    const m = new M42d();
    m._data.set(other._data);
    return m;
  }

  // element accessors (row-major: index = r*2 + c)
  get M00(): number { return this._data[0]!; }   set M00(v: number) { this._data[0] = v; }
  get M01(): number { return this._data[1]!; }   set M01(v: number) { this._data[1] = v; }
  get M10(): number { return this._data[2]!; }   set M10(v: number) { this._data[2] = v; }
  get M11(): number { return this._data[3]!; }   set M11(v: number) { this._data[3] = v; }
  get M20(): number { return this._data[4]!; }   set M20(v: number) { this._data[4] = v; }
  get M21(): number { return this._data[5]!; }   set M21(v: number) { this._data[5] = v; }
  get M30(): number { return this._data[6]!; }   set M30(v: number) { this._data[6] = v; }
  get M31(): number { return this._data[7]!; }   set M31(v: number) { this._data[7] = v; }

  /** Returns row r as a V2d (length = cols). */
  row(r: number): V2d {
    const o = r * COLS;
    return new V2d(this._data[o + 0]!, this._data[o + 1]!);
  }

  /** Returns column c as a V4d (length = rows). */
  col(c: number): V4d {
    return new V4d(this._data[0 + c]!, this._data[2 + c]!, this._data[4 + c]!, this._data[6 + c]!);
  }

  add(other: M42d): M42d {
    const m = new M42d();
    for (let i = 0; i < COMPONENT_COUNT; i++) m._data[i] = this._data[i]! + other._data[i]!;
    return m;
  }

  sub(other: M42d): M42d {
    const m = new M42d();
    for (let i = 0; i < COMPONENT_COUNT; i++) m._data[i] = this._data[i]! - other._data[i]!;
    return m;
  }

  neg(): M42d {
    const m = new M42d();
    for (let i = 0; i < COMPONENT_COUNT; i++) m._data[i] = -this._data[i]!;
    return m;
  }

  mul(scalar: number): M42d;
  mul(v: V2d): V4d;
  mul(other: V2d | number): M42d | V4d {
    if (typeof other === "number") {
      const m = new M42d();
      for (let i = 0; i < COMPONENT_COUNT; i++) m._data[i] = this._data[i]! * other;
      return m;
    }
    const a = this._data;
    const x = other._data[0]!, y = other._data[1]!;
    return new V4d(
      a[0]! * x + a[1]! * y,
      a[2]! * x + a[3]! * y,
      a[4]! * x + a[5]! * y,
      a[6]! * x + a[7]! * y,
    );
  }

  transform(v: V2d): V4d {
    return this.mul(v);
  }

  transpose(): M24d {
    const out = new M24d();
    const a = this._data, r = out._data;
    // dst M24d has 2 rows, 4 cols; row-major dst index = row * 4 + col_dst.
    // src element at (row_src, col_src) -> dst at (col_src, row_src).
    for (let row = 0; row < ROWS; row++) {
      for (let col = 0; col < COLS; col++) {
        r[col * 4 + row] = a[row * 2 + col]!;
      }
    }
    return out;
  }

  equals(other: unknown): boolean {
    if (this === other) return true;
    if (!(other instanceof M42d)) return false;
    for (let i = 0; i < COMPONENT_COUNT; i++) {
      if (this._data[i] !== other._data[i]) return false;
    }
    return true;
  }

  approxEqual(other: M42d, eps: number): boolean {
    for (let i = 0; i < COMPONENT_COUNT; i++) {
      if (Math.abs(this._data[i]! - other._data[i]!) > eps) return false;
    }
    return true;
  }

  getHashCode(): number {
    let h = hashNumber(this._data[0]!);
    for (let i = 1; i < COMPONENT_COUNT; i++) h = combineHash(h, hashNumber(this._data[i]!));
    return h;
  }

  toString(): string {
    return `M42d(${Array.from(this._data).join(", ")})`;
  }

  /** Yields elements in flat row-major order. */
  *[Symbol.iterator](): Iterator<number> {
    for (let i = 0; i < COMPONENT_COUNT; i++) yield this._data[i]!;
  }

  /** Returns a flat row-major array. */
  toArray(): number[] {
    return Array.from(this._data);
  }

  static addInto(a: M42d, b: M42d, target: M42d): M42d {
    for (let i = 0; i < COMPONENT_COUNT; i++) target._data[i] = a._data[i]! + b._data[i]!;
    return target;
  }

  static subInto(a: M42d, b: M42d, target: M42d): M42d {
    for (let i = 0; i < COMPONENT_COUNT; i++) target._data[i] = a._data[i]! - b._data[i]!;
    return target;
  }

  static mulInto(a: M42d, b: V2d, target: V4d): V4d;
  static mulInto(a: M42d, b: number, target: M42d): M42d;
  static mulInto(a: M42d, b: V2d | number, target: M42d | V4d): M42d | V4d {
    if (typeof b === "number") {
      const t = target as M42d;
      for (let i = 0; i < COMPONENT_COUNT; i++) t._data[i] = a._data[i]! * b;
      return t;
    }
    const t = target as V4d;
    const ad = a._data;
    const x = b._data[0]!, y = b._data[1]!;
    const r0 = ad[0]! * x + ad[1]! * y;
    const r1 = ad[2]! * x + ad[3]! * y;
    const r2 = ad[4]! * x + ad[5]! * y;
    const r3 = ad[6]! * x + ad[7]! * y;
    t._data[0] = r0; t._data[1] = r1; t._data[2] = r2; t._data[3] = r3;
    return t;
  }

  static copyInto(from: M42d, target: M42d): M42d {
    target._data.set(from._data);
    return target;
  }

  // ---------- operator overloads (boperators) ----------

  static "+"(a: M42d, b: M42d): M42d { return a.add(b); }
  static "-"(a: M42d, b: M42d): M42d;
  static "-"(a: M42d): M42d;
  static "-"(a: M42d, b?: M42d): M42d { return b ? a.sub(b) : a.neg(); }
  static "*"(a: M42d, b: V2d): V4d;
  static "*"(a: M42d, b: number): M42d;
  static "*"(a: number, b: M42d): M42d;
  static "*"(a: M42d | number, b: M42d | V2d | number): M42d | V4d {
    if (typeof a === "number") return (b as M42d).mul(a);
    return (a as { mul(o: V2d | number): M42d | V4d }).mul(b as V2d | number);
  }

  "+="(o: M42d): void {
    this._data[0]! += o._data[0]!;
    this._data[1]! += o._data[1]!;
    this._data[2]! += o._data[2]!;
    this._data[3]! += o._data[3]!;
    this._data[4]! += o._data[4]!;
    this._data[5]! += o._data[5]!;
    this._data[6]! += o._data[6]!;
    this._data[7]! += o._data[7]!;
  }
  "-="(o: M42d): void {
    this._data[0]! -= o._data[0]!;
    this._data[1]! -= o._data[1]!;
    this._data[2]! -= o._data[2]!;
    this._data[3]! -= o._data[3]!;
    this._data[4]! -= o._data[4]!;
    this._data[5]! -= o._data[5]!;
    this._data[6]! -= o._data[6]!;
    this._data[7]! -= o._data[7]!;
  }
  "*="(o: number): void {
    this._data[0]! *= o;
    this._data[1]! *= o;
    this._data[2]! *= o;
    this._data[3]! *= o;
    this._data[4]! *= o;
    this._data[5]! *= o;
    this._data[6]! *= o;
    this._data[7]! *= o;
  }
}
